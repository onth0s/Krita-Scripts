...
> Coverage: `python -m coverage run -m pytest -q && python -m coverage report --include="krita_pie_menu/*,filters_pie_menu/*,conditions_pie_menu/*,operations_pie_menu/*"` → ~87% overall; every operation-execution module ≥85% (bw_preview/duplicate_layer 100%), all config dialogs ≥97%. `pytest-cov` is in the `dev` extra.
