...
> Coverage: `python -m coverage run -m pytest -q && python -m coverage report --include="krita_pie_menu/*,filters_pie_menu/*,conditions_pie_menu/*,operations_pie_menu/*"` → **100%** (1,567 stmts, 0 missed; 298 tests). This includes the previously hard-to-reach tails: `find_brush_preset` substring branch (preset names with literal spaces, e.g. `"brush 0 std drw v2"`), `load_config` no-defaults, `save_config` write-failure, logger rotate/write error swallow, `sanitize_group` active-layer-inside-group path, and `direction_from_vector` NaN fall-through (the final `return None` is reachable only for NaN inputs since the 8 sector branches tile `[-180,180)` exhaustively). `pytest-cov` is in the `dev` extra.
