...
- `tests/test_sanitize_and_merge.py` — `execute_sanitize_group` purge/rename/`_top_`+B&W re-parent ordering; `execute_merge_to_black` full composite path via fake `QImage`/`QByteArray` + all guards (`_flatten_extra_checks`, not-u8, no-paint-layers, empty bounds) + WHITE/B&W re-assembly invariant.
...
> Coverage: `python -m coverage run -m pytest -q && python -m coverage report --include="krita_pie_menu/*,filters_pie_menu/*,conditions_pie_menu/*,operations_pie_menu/*"` → ~98% overall; every core/operation module ≥90% (pie_widget/toast_notification/base_extension/fit_layer/duplicate_layer/bw_preview/operations_pie_menu 100%), all config dialogs ≥97%. `pytest-cov` is in the `dev` extra.
