...
- `tests/test_geometry.py` — `direction_from_vector` deadzone + 8-sector mapping (pure, no stubs needed at runtime of the function), plus NaN fall-through to `None`.
- `tests/test_utils.py` — `load_config` deep-merge, missing/unreadable config fallback, `save_config` (incl. write-failure → `False`), condition-flag round-trip via `CONDITIONS_CONFIG_PATH` reassignment, protected/empty/u8-rgba predicates.
- `tests/test_pie_widget.py` — `SECTOR_CODES` canonical order, `SECTOR_NAMES` derivation, `_execute_sector` enable/disable/False-suppression toast behavior, validator state polling (bool/exception results), toggle-button construction, click handlers, `paintEvent` center/active/disabled/toggle paths, `trigger_selected_action`, cleanup error swallow. At 100% (via `qt_super` fixture that no-op-patches stub base event handlers so `super()` calls run headless).
...
- `tests/test_utils_api.py` — `create_incremental_layer` (incl. no-parent → root-node fallback), `resolve_action`, `find_brush_preset` (exact / space-substring / `"std drw"` fallback), `set_foreground_black`, `make_doc_active_validator` (no-doc / no-layer / extra-checks states) against fake node/doc/app objects.
...
- `tests/test_pie_interrupt.py` — AGENTS.md §9 contract: interrupt uses opacity+offscreen move (never `hide()`), is idempotent, `F11`/`Esc` interrupts on keypress, auto-repeat ignored, interrupted key-release only closes on `trigger_keys`, right-click / left-click-in-deadzone interrupt, left-click-with-direction + non-mouse-button + non-trigger-key super() fallthrough, `show_at_cursor` restores opacity+flags, deadzone/sector mouse selection with cursor origin fallback.
- `tests/test_toast_supersede.py` — `show_toast` supersede cancels the previous toast's fade timer (`stop`) before `close()` (incl. stop/close error-swallow), double-toast under `duration_ms` doesn't crash, parent/active-window/screen geometry branches, `get_left_dockers_offset` qwindow/parent-chain/no-main/exception paths. At 100%.
- `tests/test_config_dialogs.py` — `BasePieConfigDialog` (abstract `build_sector_editors` + `collect_config`, `load_current_config`, `handle_save` success = deep-merge + callback + `accept`, plus save-failure and collect-exception `QMessageBox.critical` paths); `SectorConfigDialog` / `OperationsConfigDialog` / `ConditionsConfigDialog` `collect_config` round-trips across all 8 sectors incl. condition-flag checkboxes; known-action combo pre-select via `findData`. At 100%.
- `tests/fakes.py` — shared fake layer tree (`Node`/`Group`/`Doc`/`App`/`Action`/`View`/`Window`) with Krita semantics: `childNodes()` bottom-to-top, `addChildNode(node, ref)` inserts ABOVE ref, `remove()` mutates the parent's child list (detach semantics), `Group.findChildNodes`, `Node.pixelData/setPixelData/getters`. Enables headless execution of the whole operation pipeline.
- `tests/test_duplicate_layer_and_extension.py` — `execute_duplicate_layer` no-doc/no-node/success (lock+hide original, add dup above, restore)/exception; `BasePieMenuExtension` config load/save delegation, `show_pie_menu` widget construction + already-visible early return, `_on_widget_destroyed`.
- `tests/test_sanitize_and_merge.py` — `execute_sanitize_group` purge/rename/`_top_`+B&W re-parent ordering, active-layer-inside-group path; `execute_merge_to_black` full composite path via fake `QImage`/`QByteArray` + all guards (`_flatten_extra_checks`, not-u8, no-paint-layers, empty bounds) + WHITE/B&W re-assembly invariant.
- `tests/test_refine_sketch.py` — selection cut/paste (actions, deselect fallback), HSL fill (non-U8 skip/fill/exception), duplicate-refLay, luminosity overlay (gray pixel, exceptions), execute guards and full flow with/without `duplicate_reflay`.
- `tests/test_init_canvas.py` — nuke accept/decline, single WHITE node, replace accept/decline, nuke-removal warning, brush-activation warning.
- `tests/test_fit_layer.py` — paint-layer stretch / keep-aspect-ratio / format-convert paths, group-layer per-child rescale, `_is_keep_aspect_ratio_enabled` flag, guards.
- `tests/test_bw_preview.py` — B&W toggle (top-level + nested) and create paths incl. non-4-byte pixel fill and blending-failure non-abort. At 100%.
- `tests/test_quick_script_engine.py` — `createActions` action registration + triggered wiring, `create_incremental_layer` no-doc / no-active-layer toasts, success toast, and create-returned-`None` → no-toast. At 100%.
- `tests/test_dummy_docker.py` — widget construction + button-click wiring, `canvasChanged` None / with-doc / no-doc label updates, `on_btn_click` no-doc / with-node / no-node label updates. At 100%.

> Coverage: `python -m coverage run -m pytest -q && python -m coverage report --include="krita_pie_menu/*,filters_pie_menu/*,conditions_pie_menu/*,operations_pie_menu/*,quick_script_engine/*,dummy_docker/*"` → **100%** (1,629 stmts, 0 missed; 311 tests) across all six plugins. This includes the previously hard-to-reach tails: `find_brush_preset` substring branch (preset names with literal spaces, e.g. `"brush 0 std drw v2"`), `load_config` no-defaults, `save_config` write-failure, logger rotate/write error swallow, `sanitize_group` active-layer-inside-group path, `direction_from_vector` NaN fall-through (the final `return None` is reachable only for NaN inputs since the 8 sector branches tile `[-180,180)` exhaustively), and the full `quick_script_engine` / `dummy_docker` surfaces (createActions, toast paths, canvas/active-layer label updates). `pytest-cov` is in the `dev` extra.
